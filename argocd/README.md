...
앱 레이어 app-of-apps 를 인프라 레포에서 들어내 전용 레포로 격리. *무엇을 배포할지(Application)* 는 본 GitOps 레포, *어떻게 생긴지(매니페스트)* 는 앱 레포 `deploy/k8s`, *인프라/플랫폼* 은 인프라 레포 — 세 관심사를 레포 경계로 분리. ArgoCD 공식 *config vs source code 분리* 의 연장.

### `platform` 과 분리된 `apps` AppProject

`apps` 의 `sourceRepos` 는 본 GitOps 레포 + 앱 레포(서비스별 GitHub 레포)로 한정, `destinations` 는 서비스 NS(`core`/`batch`/`login`) + Application CR 이 사는 `cicd` 로 한정. 인프라 레포가 앱 NS 를, 앱 레포가 인프라를 건드리지 못하게 권한 경계를 프로젝트로 강제.
...
`apps/<svc>.yaml` 에 Application 추가(앱 레포 `deploy/k8s` 지시) + 대상 NS 생성 + 해당 NS 에 `ghcr-pull` 복사. `apps-root` 가 auto-sync 로 흡수.
